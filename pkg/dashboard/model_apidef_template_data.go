/*
NEW Tyk DASH API

## <a name=\"introduction\"></a> Introduction      The Tyk Dashboard API offers granular, programmatic access to a centralised database of resources that your Tyk nodes can pull from. This API has a dynamic user administrative structure which means the secret key that is used to communicate with your Tyk nodes can be kept secret and access to the wider management functions can be handled on a user-by-user and organisation-by-organisation basis.      A common question around using a database-backed configuration is how to programatically add API definitions to your Tyk nodes, the Dashboard API allows much more fine-grained, secure and multi-user access to your Tyk cluster, and should be used to manage a database-backed Tyk node.      The Tyk Dashboard API works seamlessly with the Tyk Dashboard (and the two come bundled together).      ## <a name=\"security-hierarchy\"></a> Security Hierarchy      The Dashboard API provides a more structured security layer to managing Tyk nodes.      ### Organisations, APIs and Users      With the Dashboard API and a database-backed Tyk setup, (and to an extent with file-based API setups - if diligence is used in naming an creating definitions), the following security model is applied to the management of Upstream APIs:      * **Organisations**: All APIs are *owned* by an organisation, this is designated by the OrgID parameter in the API Definition.     * **Users**: All users created in the Dashboard belong to an organisation (unless an exception is made for super-administrative access).     * **APIs**: All APIs belong to an Organisation and only Users that belong to that organisation can see the analytics for those APIs and manage their configurations.     * **API Keys**: API Keys are designated by organisation, this means an API key that has full access rights will not be allowed to access the APIs of another organisation on the same system, but can have full access to all APIs within the organisation.     * **Access Rights**: Access rights are stored with the key, this enables a key to give access to multiple APIs, this is defined by the session object in the core Tyk API.      In order to use the Dashboard API, you'll need to get the Tyk Dashboard API Access Credentials secret from your user profile on the Dashboard UI.      The secret you set should then be sent along as a header with each Dashboard API Request in order for it to be successful:  authorization: <your-secret>

API version: 5.4.0
Contact: support@tyk.io
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package dashboard

import (
	"encoding/json"
)

// checks if the ApidefTemplateData type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ApidefTemplateData{}

// ApidefTemplateData struct for ApidefTemplateData
type ApidefTemplateData struct {
	EnableSession  *bool   `json:"enable_session,omitempty"`
	InputType      *string `json:"input_type,omitempty"`
	TemplateMode   *string `json:"template_mode,omitempty"`
	TemplateSource *string `json:"template_source,omitempty"`
}

// NewApidefTemplateData instantiates a new ApidefTemplateData object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewApidefTemplateData() *ApidefTemplateData {
	this := ApidefTemplateData{}
	return &this
}

// NewApidefTemplateDataWithDefaults instantiates a new ApidefTemplateData object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewApidefTemplateDataWithDefaults() *ApidefTemplateData {
	this := ApidefTemplateData{}
	return &this
}

// GetEnableSession returns the EnableSession field value if set, zero value otherwise.
func (o *ApidefTemplateData) GetEnableSession() bool {
	if o == nil || IsNil(o.EnableSession) {
		var ret bool
		return ret
	}
	return *o.EnableSession
}

// GetEnableSessionOk returns a tuple with the EnableSession field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ApidefTemplateData) GetEnableSessionOk() (*bool, bool) {
	if o == nil || IsNil(o.EnableSession) {
		return nil, false
	}
	return o.EnableSession, true
}

// HasEnableSession returns a boolean if a field has been set.
func (o *ApidefTemplateData) HasEnableSession() bool {
	if o != nil && !IsNil(o.EnableSession) {
		return true
	}

	return false
}

// SetEnableSession gets a reference to the given bool and assigns it to the EnableSession field.
func (o *ApidefTemplateData) SetEnableSession(v bool) {
	o.EnableSession = &v
}

// GetInputType returns the InputType field value if set, zero value otherwise.
func (o *ApidefTemplateData) GetInputType() string {
	if o == nil || IsNil(o.InputType) {
		var ret string
		return ret
	}
	return *o.InputType
}

// GetInputTypeOk returns a tuple with the InputType field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ApidefTemplateData) GetInputTypeOk() (*string, bool) {
	if o == nil || IsNil(o.InputType) {
		return nil, false
	}
	return o.InputType, true
}

// HasInputType returns a boolean if a field has been set.
func (o *ApidefTemplateData) HasInputType() bool {
	if o != nil && !IsNil(o.InputType) {
		return true
	}

	return false
}

// SetInputType gets a reference to the given string and assigns it to the InputType field.
func (o *ApidefTemplateData) SetInputType(v string) {
	o.InputType = &v
}

// GetTemplateMode returns the TemplateMode field value if set, zero value otherwise.
func (o *ApidefTemplateData) GetTemplateMode() string {
	if o == nil || IsNil(o.TemplateMode) {
		var ret string
		return ret
	}
	return *o.TemplateMode
}

// GetTemplateModeOk returns a tuple with the TemplateMode field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ApidefTemplateData) GetTemplateModeOk() (*string, bool) {
	if o == nil || IsNil(o.TemplateMode) {
		return nil, false
	}
	return o.TemplateMode, true
}

// HasTemplateMode returns a boolean if a field has been set.
func (o *ApidefTemplateData) HasTemplateMode() bool {
	if o != nil && !IsNil(o.TemplateMode) {
		return true
	}

	return false
}

// SetTemplateMode gets a reference to the given string and assigns it to the TemplateMode field.
func (o *ApidefTemplateData) SetTemplateMode(v string) {
	o.TemplateMode = &v
}

// GetTemplateSource returns the TemplateSource field value if set, zero value otherwise.
func (o *ApidefTemplateData) GetTemplateSource() string {
	if o == nil || IsNil(o.TemplateSource) {
		var ret string
		return ret
	}
	return *o.TemplateSource
}

// GetTemplateSourceOk returns a tuple with the TemplateSource field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ApidefTemplateData) GetTemplateSourceOk() (*string, bool) {
	if o == nil || IsNil(o.TemplateSource) {
		return nil, false
	}
	return o.TemplateSource, true
}

// HasTemplateSource returns a boolean if a field has been set.
func (o *ApidefTemplateData) HasTemplateSource() bool {
	if o != nil && !IsNil(o.TemplateSource) {
		return true
	}

	return false
}

// SetTemplateSource gets a reference to the given string and assigns it to the TemplateSource field.
func (o *ApidefTemplateData) SetTemplateSource(v string) {
	o.TemplateSource = &v
}

func (o ApidefTemplateData) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ApidefTemplateData) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.EnableSession) {
		toSerialize["enable_session"] = o.EnableSession
	}
	if !IsNil(o.InputType) {
		toSerialize["input_type"] = o.InputType
	}
	if !IsNil(o.TemplateMode) {
		toSerialize["template_mode"] = o.TemplateMode
	}
	if !IsNil(o.TemplateSource) {
		toSerialize["template_source"] = o.TemplateSource
	}
	return toSerialize, nil
}

type NullableApidefTemplateData struct {
	value *ApidefTemplateData
	isSet bool
}

func (v NullableApidefTemplateData) Get() *ApidefTemplateData {
	return v.value
}

func (v *NullableApidefTemplateData) Set(val *ApidefTemplateData) {
	v.value = val
	v.isSet = true
}

func (v NullableApidefTemplateData) IsSet() bool {
	return v.isSet
}

func (v *NullableApidefTemplateData) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableApidefTemplateData(val *ApidefTemplateData) *NullableApidefTemplateData {
	return &NullableApidefTemplateData{value: val, isSet: true}
}

func (v NullableApidefTemplateData) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableApidefTemplateData) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
